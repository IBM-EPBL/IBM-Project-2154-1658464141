...
@app.route('/register') # routes to the register page
def register():
    return render_template('register.html')#rendering the register page

@app.route('/gesture',methods=['GET','POST'])# routes to the index html
def gesture():
    ...
    if request.method == 'POST':
        ...
        while True:
            ...
            if prediction[0][0]=='ONE':
                ...
                half = cv2.resize(image1, (0, 0), fx = 0.1, fy = 0.1)
                sc = cv2.resize(image1, (200, 150),interpolation = cv2.INTER_AREA)
                resized = cv2.resize(image1, (120, 120))
                pre_img=resized
                cv2.imshow("Scale Down", sc)
                key=cv2.waitKey(3000)
                
                if (key & 0xFF) == ord("1"):
                    cv2.destroyWindow("Scale Dwon")
            
            elif prediction[0][0]=='ZERO':
                
                #cv2.rectangle(image1, (480, 170), (650, 420), (0, 0, 255), 2)
                cv2.imshow("Undo", pre_img)
                cv2.waitKey(0)
                key=cv2.waitKey(3000)
                if (key & 0xFF) == ord("0"):
                    cv2.destroyWindow("Undo")
                ...
                pre_img=resized
                cv2.imshow("Enlarge", resized)
                key=cv2.waitKey(3000)
                if (key & 0xFF) == ord("4"):
                    cv2.destroyWindow("Enlarge")
